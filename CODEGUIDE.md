...
- Worker 负责抓取、Markdown 正文采集、item normalize、可解释分析、反馈归纳、简报生成和 source quality observation；常驻 Queue Worker 只消费 durable command，不运行全量 cron pipeline。AI 摘要必须在正文采集 `READY` 后执行。
