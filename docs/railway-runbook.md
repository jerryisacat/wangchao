...
四个 Railway application service 分别使用独立的 Config as Code 文件：
...
4. 测试：push 一个小改动到 `master`，确认 Railway 为五个 service 各创建一个 deployment。

### 1.3 哪些 commit 会触发自动部署

**所有 push 到 `master` 分支的 commit 都会触发 Railway 自动部署**（五个 service 同时）。因此：
...
当前五个 Railway service 的 build command 均为 `pnpm railway:build`（完整 monorepo 构建）。这是刻意选择的安全路径：
...
| Variable | Web | Worker Cron | Source Discovery | Report Cron | Source |
|---|---|---|---|---|---|
| `DATABASE_URL` | ✅ service ref | ✅ service ref | ✅ service ref | ✅ service ref | Railway Postgres |
| `WANGCHAO_DEFAULT_ORGANIZATION_SLUG` | ✅ | ✅ | ✅ | ✅ | 手动设置 |
| `WANGCHAO_DEFAULT_ORGANIZATION_NAME` | ✅ | ✅ | ✅ | ✅ | 手动设置 |
| `WANGCHAO_DEFAULT_USER_EMAIL` | ✅ | ✅ | ✅ | ✅ | 手动设置 |
| `WANGCHAO_DEFAULT_USER_NAME` | ✅ | ✅ | ✅ | ✅ | 手动设置 |
| `ENCRYPTION_KEY` | ✅ | ✅ | ✅ | ✅ | 手动设置（AES-256-GCM） |
| `AI_BASE_URL` | — | ✅ | — | ✅ | Admin 后台优先 |
| `AI_API_KEY` | — | ✅ | — | ✅ | Admin 后台优先 |
| `AI_MODEL_L1` | — | ✅ | ✅ | ✅ | Admin 后台优先 |
| `AI_MODEL_L2` | — | ✅ | — | ✅ | Admin 后台优先 |
| `BRAVE_SEARCH_API_KEY` | — | — | ✅ | — | Admin 后台优先 |
| `WANGCHAO_SEARCH_PROVIDER` | — | — | ✅ | — | 手动设置 |
| `WANGCHAO_DISCOVERY_*` | — | — | ✅ | — | 手动设置（可选，有默认值） |
| `WANGCHAO_FETCH_CONCURRENCY` | — | ✅ | — | — | 手动设置（可选，有默认值） |
| `WANGCHAO_FETCH_BACKOFF_BASE_MS` | — | ✅ | — | — | 手动设置（可选，有默认值） |
| `WANGCHAO_CANDIDATE_OBSERVATION_ENABLED` | — | ✅ | — | — | 手动设置（可选，默认 false） |
| `WANGCHAO_CANDIDATE_OBSERVATION_CONCURRENCY` | — | ✅ | — | — | 手动设置（可选，默认 3） |
| `WANGCHAO_SEED_SOURCE_NAME` | ✅ | — | — | — | 手动设置（首次部署） |
| `WANGCHAO_SEED_SOURCE_URL` | ✅ | — | — | — | 手动设置（首次部署） |
| `WANGCHAO_SEED_SOURCES_URL` | ✅ | — | — | — | 手动设置（可选） |
| `WANGCHAO_TOPIC_CREATE_*` | ✅ | — | — | — | 手动设置（可选，有默认值） |
| `WANGCHAO_DB_WAIT_*` | ✅ | ✅ | ✅ | ✅ | 手动设置（可选，有默认值） |
| `WANGCHAO_RAILWAY_ROLE` | ✅ (root config only) | ✅ (root config only) | ✅ (root config only) | ✅ (root config only) | 仅 `railway up` fallback 使用 |
| `PORT` | ✅ (Railway injected) | — | — | — | Railway 自动注入 |
...
3. 在 Web、Worker Cron、Source Discovery Cron 五个 service 的 **Variables** 中，使用 **Reference Variable** 引用 Postgres service 的 `DATABASE_URL`（格式：`${{Postgres.DATABASE_URL}}`）。
