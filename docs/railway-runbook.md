...
4. 测试：push 一个小改动到 `master`，确认 Railway 为三个 service 各创建一个 deployment。

### 1.3 哪些 commit 会触发自动部署

**所有 push 到 `master` 分支的 commit 都会触发 Railway 自动部署**（三个 service 同时）。因此：
...
当前三个 Railway service 的 build command 均为 `pnpm railway:build`（完整 monorepo 构建）。这是刻意选择的安全路径：
...
| Variable | Web | Worker Cron | Source Discovery | Source |
|---|---|---|---|---|
| `DATABASE_URL` | ✅ service ref | ✅ service ref | ✅ service ref | Railway Postgres |
...
| `WANGCHAO_RAILWAY_ROLE` | ✅ (root config only) | ✅ (root config only) | ✅ (root config only) | 仅 `railway up` fallback 使用 |
...
3. 在 Web、Worker Cron、Source Discovery Cron 三个 service 的 **Variables** 中，使用 **Reference Variable** 引用 Postgres service 的 `DATABASE_URL`（格式：`${{Postgres.DATABASE_URL}}`）。
