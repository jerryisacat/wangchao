...
| `packages/db/src/repositories.fixtures.ts` | Repository fixture：验证收藏集合查询的 organization/user scope、分页边界和越界页收敛。 |
...
| `apps/web/src/lib/topic-source-data.ts` | 读取工作台、完整收藏分页和单条情报详情；`DATABASE_URL` 未配置时抛出错误，不再静默降级为预览模式；统一调用 `event-display.ts` 清洗展示字段。 |
