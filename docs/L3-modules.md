...
│   ├── db/                           # Postgres/Prisma schema、migration、client、repositories、加密模块（crypto.ts）
│   ├── sources/                      # RSS/Web source adapter（含 ssrf.ts 防护）
│   └── ui/                           # 共享 UI 包（预留）
├── tests/
...
| `packages/db/prisma/migrations/0012_instant_push/migration.sql` | 新增 InstantPushLog 状态机、订阅开关和即时推送 task/usage 枚举。 |
| `packages/db/prisma/migrations/0013_credentials_split/migration.sql` | 拆分 Subscription 凭证到 OrganizationCredential；将 Telegram 即时推送开关迁移到 `OrganizationCredential(TELEGRAM)` 并删除 Subscription 旧字段；删除冗余字段（secondaryItemIds/idempotencyKey/dismissedAt）；PaymentInvoice.amount Float→Decimal；Session.expiresAt 索引；FeedbackEvent 复合索引；migration 0002 IF NOT EXISTS 修复。 |
| `packages/db/prisma/migrations/0015_content_capture_status/migration.sql` | 新增 Item 正文采集状态/来源/时间/错误码、IntelligenceEvent 摘要状态/重试时间和 `CONTENT_FETCH` TaskRun 类型；既有非空 `rawContent` 标记为 `READY/LEGACY_TEXT`。 |
| `packages/db/prisma/migrations/0016_better_auth_schema_alignment/migration.sql` | Better Auth 1.6.23 core schema 对齐 + Issue #153 Lane 1 user lifecycle 数据层。新增 `Verification` 模型、`UserAccountStatus` 枚举、User/Account 生命周期与认证字段。 |
| `packages/db/prisma/migrations/0017_task_run_lease_queue/migration.sql` | Issue #162 durable TaskRun queue：新增 idempotency/lease/heartbeat 字段、due-scan index，以及仅约束 `PENDING/RUNNING` 的 tenant/type/key partial unique index；不篡改历史 RUNNING 行。 |
...
| `packages/db/src/extended-repositories.ts` | Barrel re-export，将 9 个领域子模块的导出统一暴露给 `@wangchao/db`。 |
| `packages/db/src/repositories/instant-push.ts` | InstantPushLog 状态机、`OrganizationCredential(TELEGRAM)` 即时推送设置、候选查询、原子 claim、标记成功/失败。 |
| `packages/db/src/repositories/telegram-credential.ts` | Telegram 凭证 view/upsert/enable/delete/decrypt/test。 |
| `packages/db/src/repositories/delivery-log.ts` | DeliveryLog 幂等创建、状态更新、待投递简报查询。 |
| `packages/db/src/repositories/report.ts` | Report CRUD、完成/失败、pending 扫描、证据事件检索。 |
| `packages/db/src/repositories/preference-memory.ts` | 偏好记忆删除/权重更新/增强反馈记录。 |
| `packages/db/src/repositories/byok-credential.ts` | BYOK 凭证 view/upsert/delete/decrypt。 |
| `packages/db/src/repositories/subscription.ts` | 订阅 plan view/update、self-hosted 模式、用量计数。 |
| `packages/db/src/repositories/ccpayment-credential.ts` | CCPayment 凭证 view/upsert/delete/decrypt。 |
| `packages/db/src/repositories/payment-invoice.ts` | 支付订单创建/查询/状态更新。 |
| `packages/db/src/crypto.ts` | AES-256-GCM 凭证加解密工具。`encryptCredential(plaintext, key)` / `decryptCredential(encrypted, key)` / `maskKeyHint(plaintext)` / `fingerprintKey(key)` / `cryptoSmokeTest()`。per-credential 随机 salt + scrypt KDF，加密和解密使用同一 salt 派生 key；密文格式 `salt:iv:ciphertext:tag`（base64，旧格式 `iv:ciphertext:tag` 保持向后兼容）。`decryptCredential` 用窄 authenticated-decrypt helper（`tryAuthenticatedDecrypt`）：在 component 长度/base64/encryptionKey 前置校验通过后，仅围绕 `decipher.final()` 捕获认证失败，返回 discriminated result；deriveKey/createDecipheriv/setAuthTag/update 的异常不被 fallback 吞掉。四段旧 bug 密文在 stored salt 认证失败时尝试 STATIC_SALT fallback，仅限旧密文。两次认证均失败时抛固定安全错误 `"Credential decryption failed"`，不泄露 Node/OpenSSL 内部字符串；格式/长度/base64 错误仍保留明确 `Invalid...` 类消息。入口检查 `MAX_ENCRYPTED_CREDENTIAL_LENGTH`（16384 bytes，注释说明依据）超限立即抛固定错误，不执行 base64/KDF。严格校验 component 长度（salt=16/iv=12/tag=16）和 canonical base64（`decodeBase64Strict`）。`STATIC_SALT` 不导出为公共 API。`cryptoSmokeTest()` 纳入 `repositories.fixtures.ts` 测试套件执行。 |
| `packages/db/src/index.ts` | 包公共出口。导出 repositories、crypto 函数以及凭证类型。`extended-repositories.ts` 现为 barrel re-export，拆分为 9 个领域子模块（见上表）。 |
| `packages/db/src/repositories.ts` | Repository 入口文件。按领域拆分为子模块后纯 re-export，所有函数签名不变。 |
| `packages/db/src/repositories/types.ts` | 所有公共接口和类型定义（~509 行），包括 TenantScope、TopicScope、DashboardEventRecord 等。 |
| `packages/db/src/repositories/workspace.ts` | `ensureDefaultWorkspace` 初始化兼容模式默认工作区；`ensureUserWorkspace` 在同一事务内读取用户最早 Membership，存在则复用，否则按 `SHA-256(userId)` 确定性 slug upsert 独立 Organization + OWNER Membership；真正的并发幂等由两个 upsert 唯一键保证。 |
| `packages/db/src/repositories/webhook-event.ts` | Webhook 事件幂等：`claimWebhookEvent` 通过 `provider + recordId` 唯一约束 catch `P2002` 实现去重；`isWebhookRecordProcessed` 查询去重状态。 |
| `packages/db/src/repositories/topic.ts` | Topic CRUD 和 organization membership 管理。 |
| `packages/db/src/repositories/source.ts` | 信源管理、治理、RSS 抓取、TaskRun、fetch items。`upsertFetchedItems` 已批量化（createMany+updateMany in $transaction）。 |
| `packages/db/src/repositories/event.ts` | 情报事件、dashboard、timeline、briefing、feedback、preference、merge。事务含 organizationId 防御性守卫。 |
| `packages/db/src/repositories/export.ts` | ExportEvent、FeedbackEvent、UsageEvent、Subscription 凭证管理。`recordMarkdownExport` 已包在同一 `$transaction`。 |
| `packages/db/src/repositories/secondary-sources.ts` | 共享 secondary sources 解析器 `resolveSecondarySources`，供 briefing 和 timeline 共用。 |
| `packages/db/src/repositories/util.ts` | 共享工具函数：canonicalizeUrl、readRuntimeEnv、toInputJson 等。 |
| `packages/db/src/repositories/user-lifecycle.ts` | Issue #153 Lane 1B-1 用户生命周期状态机。原子转换、稳定错误码与登录/活跃时间戳职责分离。 |
| `packages/db/src/repositories/task-run.ts` | Issue #162 durable queue repository。提供 active-idempotent enqueue、PostgreSQL `SKIP LOCKED` claim、lease renew、fenced complete/fail/yield，以及过期 lease bounded reaper；所有 raw SQL 使用参数化 `Prisma.sql`。 |
| `packages/db/src/task-run-schema.fixtures.ts` / `task-run.fixtures.ts` | 无数据库 schema/migration 与 repository 契约 fixtures；验证 partial unique 不被误建为全局 unique、fencing SQL、UTF-8 payload 上限和固定 error class。 |
| `packages/db/src/task-run-pg.fixtures.ts` | 显式 opt-in disposable PostgreSQL 并发 suite：并发 enqueue winner、SKIP LOCKED claim 唯一性、stale token、retry/yield/reaper 与终态 key 复用。 |
...
| `packages/db/src/user-lifecycle.fixtures.ts` | Issue #153 用户生命周期状态机 fixture（纯单元测试，无需 DATABASE_URL），验证转换、错误码、`now` 注入和原子 `updateMany` 谓词；schema 契约验证拆到 `user-lifecycle-schema.fixtures.ts`，使单文件保持低于 800 行。 |
| `packages/db/src/user-lifecycle-schema.fixtures.ts` | Better Auth/User lifecycle Prisma DMMF 契约 fixture：验证 `User.name` NOT NULL、core/OAuth 字段、生命周期字段和 `UserAccountStatus` 四值。 |
| `packages/db/src/workspace-auth.fixtures.ts` | `ensureUserWorkspace` fixture：覆盖既有 Membership 复用、确定性 hashed slug、OWNER Membership、不同用户组织隔离与空白名称回退。 |
| `packages/db/src/migration-replay.fixtures.ts` | Issue #153 专用 migration replay fixture。仅由 `pnpm --filter @wangchao/db test:migration-replay` 显式运行；调用者在 disposable PostgreSQL 的 0015 状态插入 `preexisting-replay-user` 后应用 0016，用于验证 PG enum、NOT NULL 回填、Better Auth 字段和新旧用户默认值。普通 `pnpm test` 不读取 `DATABASE_URL` 触发 replay。 |
...
| 函数 | 职责 |
|------|------|
| `ensureDefaultWorkspace()` / `ensureUserWorkspace()` | 前者创建兼容模式默认 organization/user/membership；后者为认证用户原子复用或创建独立 OWNER workspace。 |
...
| `listTopicSourceOverview()` / `listSourceGovernanceReport()` | 信源治理报告；后者读 Source 持久化 qualityScore（为 0 时回退派生值），按 active primary/secondary EventItem 关系计算 hit/noise/duplicate 与唯一活跃事件数，暴露 persisted/derived/stale 三值。 |
| `updateSourceGovernanceStatus()` / `recordSourceQualityObservation()` | 信源治理操作。`recordSourceQualityObservation` 在单一 `$transaction` 内写 SourceObservation 历史 + 更新 Source.qualityScore，evidence 记录 formulaVersion；trustScore 不被 observation 自动改（SPEC §5.2 边界）。 |
| `getSourceQualitySummary()` | 统一读取接口：返回 Source 当前 qualityScore/trustScore/status/totalItems，供事件评分、候选晋升和信源调度使用同一份质量视图（#176）。 |
...
| `createTaskRun()` / `createSourceFetchTaskRun()` / `createSourceDiscoveryTaskRun()` / `completeTaskRun()` / `failTaskRun()` | 同步审计型 TaskRun helper，供已经在当前进程执行的细粒度 AI/采集工作记录 RUNNING→终态；不用于 Web durable producer。失败统一经 `classifyTaskRunError()` 写固定低基数 class，不保存 raw message/URL/stack。 |
| `enqueueTaskRun()` / `enqueueTaskRunWithMutation()` / `claimNextTaskRun()` / `renewTaskRunLease()` / `completeClaimedTaskRun()` / `failClaimedTaskRun()` / `yieldClaimedTaskRun()` / `recoverExpiredTaskRuns()` | Durable queue public API。Web enqueue PENDING；带 mutation 版本把任务创建与 UI-visible PENDING 状态放在同一事务，避免快速 consumer 竞态；Worker 原子 claim + heartbeat；所有 settlement 受 owner/token/expiry fencing。 |
| `recordSourceFetchSuccess()` / `upsertFetchedItems()` | 抓取结果写入。 |
| `listFetchedItemsForAnalysis()` / `markItemFiltered()` / `upsertIntelligenceEventFromItem()` | 分析管线数据流；只读取 ACTIVE source；标题模糊命中按已有 event id 更新，维护唯一 PRIMARY EventItem，并把旧 primary 标记为 SECONDARY/DUPLICATE。 |
| `listItemsPendingContentCapture()` / `updateItemContentCapture()` | 正文采集队列与结构化结果写入；`rawContent` 保存安全 Markdown。 |
...
| `listRecentFeedbackSignals()` / `generatePreferenceDeltas()` / `upsertPreferenceMemory()` / `listPreferenceMemoryForDashboard()` | 偏好学习数据流；查询纳入全部 12 种非治理反馈类型（READ/SAVE/DISMISS/EXPORT + 6 种增强反馈），mapper 返回 `feedbackEventId`/`eventId`/`createdAt` 完整字段，core 按 `feedbackEventId` 主键幂等 + `topicId` + preference key 隔离归纳，30 天半衰期时间衰减。 |
...
| 文件 | 目的 |
|------|------|
| `packages/core/src/index.ts` | Barrel re-export，将 12 个领域子模块的导出统一暴露给 `@wangchao/core`。 |
...
| `packages/core/src/env.ts` | 环境变量读取 helper：`readPositiveIntegerEnv` / `readFloatEnv` / `readBoundedNumberEnv`。 |
| `packages/core/src/text.ts` | 文本工具：`isHttpUrl` / `stripHtml`。 |
| `packages/core/src/date-range.ts` | UTC 日期范围：`createUtcDayRange` / `createUtcWeekRange` / `createUtcMonthRange`。 |
| `packages/core/src/hashing.ts` | 内容哈希：`createContentHash`（surrogate-pair-safe FNV-1a）。 |
| `packages/core/src/topic-profile.ts` | Topic profile 初稿生成：`buildTopicProfile` / `buildTopicProfileContext` / `extractKeywords`。 |
| `packages/core/src/relevance.ts` | Relevance/noise 判定与事件草稿：`evaluateRelevance` / `createIntelligenceEventDraft` / `createIntelligenceEventDraftFromExtraction` / `calculateGravityScore`。 |
| `packages/core/src/preference.ts` | 偏好学习：`generatePreferenceDeltas` / `applyPreferenceWeights` / `preferenceKeysForEvent`。 |
| `packages/core/src/render-event.ts` | 单条情报 Markdown 渲染：`renderEventMarkdown`。 |
| `packages/core/src/render-briefing.ts` | 简报 Markdown 渲染：`renderDailyBriefingMarkdown` / `renderPeriodBriefingMarkdown`。 |
...
| `generatePreferenceDeltas(signals)` | 把 FeedbackEvent 归纳成 PreferenceMemory delta。按 `feedbackEventId` 主键幂等（同一 FeedbackEvent replay 不重复计入），`topicId` 纳入 dedupKey 防止跨 Topic 吞信号；偏好信号带 30 天半衰期时间衰减（`applyTimeDecay`），旧信号自动衰减权重。`signalCount` 为去重后原始计数，`weight` 为衰减后累积值。 |
...
| 文件 | 目的 |
|------|------|
| `packages/sources/src/index.ts` | 包公共出口：RSS source adapter、search provider、feed validation/probe、外链提取、`fetchArticleMarkdown()`（Readability `article.content` + 安全 HTML→Markdown）、`htmlToSafeMarkdown()`、专用 source adapters 和 search provider 工厂。X/Twitter 当前明确返回 `UNSUPPORTED`，不使用普通网页抓取冒充平台正文。 |
...
| `packages/sources/src/ssrf.ts` | SSRF 防护工具：`isPrivateIP` / `resolveAndCheckUrl` / `assertSafeUrl`。阻断私有 IP、loopback、cloud metadata（169.254.169.254 等）。所有外部 URL fetch 前必须通过此模块。 |

### 规则

- 仅接受 HTTP/HTTPS URL。fetch 前必须经过 `assertSafeUrl()` 阻断私有 IP / loopback / cloud metadata。
...
- RSS `content:encoded` 会先转换为安全 Markdown，再随 ACTIVE/candidate 两条 upsert 路径写入 `rawContent`；无 embedded content 的 ACTIVE Item 由 `fetchArticleMarkdown()` 异步采集。Markdown 转换丢弃 script/style/embed/form、事件处理器和 javascript URL。

---

## apps/web

Next.js App Router 产品界面。按 `FRONTEND.md` 采用 Material You（MD3）视觉体系：公开营销页使用 1200px 编辑式情报画布，产品工作台使用 920px 限宽单列阅读区；两者共享暖白 tonal 表面与紫色主强调，但由 Marketing/Auth/Product 三类 Shell 隔离数据和导航边界。
...
│   │   ├── topics.ts                 # 主题 CRUD actions
│   │   ├── sources.ts                # 信源/发现/治理 actions
│   │   ├── events.ts                 # 情报状态/反馈/摘要重生成 actions
│   │   ├── credentials.ts            # AI/Search/Telegram/CCPayment/BYOK 凭证 actions
...
│   ├── layout/
│   │   ├── app-shell.tsx             # Marketing/Auth/Product 三类壳与响应式内容容器
│   │   ├── top-nav.tsx               # MD3 顶部导航（4 个阅读入口 + 新增主题 + 更多分组菜单）
│   │   └── workspace-switcher.tsx    # Radix DropdownMenu 工作区切换（焦点/Esc/长名称）
│   ├── marketing/
│   │   ├── marketing-nav.tsx         # 公开导航与登录态 CTA
│   │   ├── marketing-noscript-fallback.tsx # AppShell 直出的无 JS 核心内容
│   │   ├── intelligence-demo.tsx     # 静态 fixture 四阶段自动演示 + reduced-motion
│   │   └── tidal-contours.tsx        # 无障碍隐藏的 SVG 潮汐等高线
│   ├── intelligence/
...
└── lib/
    ...
    ├── session.ts                    # Session helper：无 Cookie 公开短路、请求级认证 Session 复用、认证用户 workspace；兼容模式默认 workspace
...
| `apps/web/src/app/page.tsx` | 公开落地页 Server Component：编辑式 Hero、渐进式潮汐演示、三项核心价值、最终 CTA 与品牌 Footer；仅解析访问状态以生成 CTA，匿名无 Session Cookie 时不加载 Auth/Prisma。 |
| `apps/web/src/app/app/page.tsx` | 工作台首页：未读情报流，顶部搜索、主题筛选、`view=all\|high\|saved` 视图、情报卡片列表、已读/收藏/减少动作；卡片来源名称链接到 Source URL，“原文”动作只链接到清洗后的 Item/Article URL，无原文时降级显示“来源”。 |
| `apps/web/src/components/marketing/intelligence-demo.tsx` | 客户端仅负责可见性与时间轴：进入视口播放一次、页面隐藏暂停、最终停留/重播；静态 DOM 已含最终情报，reduced-motion 直接进入终态，不访问 API/AI/数据库。 |
| `apps/web/src/components/marketing/intelligence-demo-shell.tsx` | 首屏等高静态预览与渐进加载边界；达到 55% 可见后才下载互动演示 chunk，避免首屏 hydration/layout 成本并保持 CLS=0。 |
| `apps/web/src/components/marketing/marketing-noscript-fallback.tsx` | 由 Marketing AppShell 在 `/` 的初始服务端 shell 中直接输出；禁用 JavaScript 时提供 Hero、最终情报、三项价值与 CTA，避免 Next RSC children 未挂载时只剩导航。 |
| `apps/web/src/lib/web-routes.ts` | 三类 Shell 纯策略、受控 pathname header 名、`APP_HOME_PATH=/app` 与旧首页查询迁移；只保留 `q/topic/view`，营销 UTM 不触发跳转；Server Action returnTo 必须属于 Product Shell，营销/Auth 路径与外部路径回退 `/app`。 |
| `apps/web/src/app/events/[eventId]/page.tsx` | 单条情报详情页：稳定 URL、摘要状态提示、来源/时间/解释、类别人类化、分组反馈、durable 异步重新采集、Markdown 导出和原文链接。 |
| `apps/web/src/app/actions/events.ts` | 情报状态、类别/增强反馈与摘要重生成 Server Actions；摘要重生成以 event-scoped active key 原子写入 `CONTENT_FETCH` PENDING 任务和等待状态，不执行网络/AI。 |
...
| `apps/web/src/lib/task-run-enqueue.ts` | 手动 SOURCE_FETCH/SOURCE_DISCOVERY 的 60 秒 UTC 时间桶 key，以及摘要重生成的 event-scoped active key；抑制双击、请求重试和运行中重复重置。 |
...
| `apps/web/src/app/topics/[topicId]/edit/page.tsx` | 主题编辑页：按“基本信息 / 主题画像 / 语言与简报偏好”拆分工作卡片，编辑 name/description 与 keywords/entities/includeScope/excludeScope/importanceRules；摘要语言在 i18n 前只读显示为简体中文，术语规则仍可编辑；每项有长度/条数边界，保存后重定向回详情页。 |
...
| `apps/web/src/lib/auth-access.ts` | 纯认证访问策略：`/` 与 `/pricing` 公开、`/app` 受保护；`normalizeAuthReturnPath()` 仅允许站内绝对 path，拒绝绝对 URL、`//`、反斜杠和控制字符，缺省回到 `/app`；`buildLoginPath()` 生成编码后的 `/login?next=`。 |
| `apps/web/src/lib/session.ts` | `getSessionWorkspace()`：认证模式验证数据库 Session 后调用 `ensureUserWorkspace()`，缺失/过期时抛稳定 `UNAUTHENTICATED`；兼容模式调用 `ensureDefaultWorkspace()`。`hasAuthenticatedSession()` 仅解析 Session、不查询/创建工作区，供 Marketing CTA 使用；`getOptionalSessionWorkspace()` 只在认证模式且存在 Session 时解析工作区，供公开定价叠加当前方案，匿名与 self-hosted 请求返回 `null`。 |
| `apps/web/src/proxy.ts` | Next.js 16 proxy（原 `middleware.ts`）。`/` 的旧 `q/topic/view` 查询在渲染前 307 到 `/app`，营销 UTM 保留；认证启用时，除 `/`、login/register/pricing、auth/health 和支付 webhook 外，每个请求均通过 Better Auth `getSession()` 验证数据库 Session：页面无 Session 时 307 到安全 `/login?next=`；受保护 API/Server Action 返回 `401 UNAUTHENTICATED`；认证服务故障返回 `503 AUTH_UNAVAILABLE`。允许渲染的请求会覆盖写入受控 pathname header，供 Server AppShell 选择 Marketing/Auth/Product 壳；认证关闭时跳过 gate。production 继续为 request/response 注入 nonce CSP，并让 redirect/401/503 保留 HSTS、X-Content-Type-Options、X-Frame-Options、Referrer-Policy、Permissions-Policy。开发环境跳过 CSP 以兼容 HMR。 |
...
| 文件 | 目的 |
|------|------|
| `apps/worker/src/index.ts` | Worker 入口。默认主 cycle 只初始化一次总预算，先 drain durable TaskRun，再把剩余预算交给多组织 fetch orchestrator；`--task-runs` 仅执行 queue consumer，其他 CLI cycle 不变。 |
| `apps/worker/src/modules/queue-worker.ts` | 常驻 durable queue runner：持续 drain `TaskRun`、空闲轮询、每轮刷新生命周期预算、结构化 heartbeat，并在 SIGTERM/SIGINT 后停止 claim 新任务、等待当前任务受控收尾。 |
| `apps/worker/src/modules/queue-worker.fixtures.ts` | 常驻队列 fixture：覆盖空闲轮询、积压队列连续 drain、预算刷新、heartbeat 聚合指标和 graceful shutdown。 |
| `apps/worker/src/modules/task-run-consumer.ts` | Exact-type SOURCE_FETCH/SOURCE_DISCOVERY/CONTENT_FETCH consumer：reaper、bounded claim、按 type 严格解析 input、workspace/单事件 dispatch、lease heartbeat、指数退避和 fenced settlement。 |
| `apps/worker/src/modules/summary-regeneration.ts` | 手动摘要单事件 handler：校验 tenant 与 task 绑定的 topic/item/event，复用 RSS Markdown 或仅抓取指定 URL，执行一次 AI extraction，持久化结构化摘要状态和用量；不扫描 Top-20 pending items，不运行完整 fetch cycle。 |
| `apps/worker/src/modules/summary-regeneration.fixtures.ts` | 单事件摘要 fixture：覆盖 RSS Markdown 复用、一次 AI 调用、READY 写回，以及正文不足时禁止调用 AI 并写结构化状态。 |
| `apps/worker/src/modules/task-run-consumer.fixtures.ts` | 无 DB consumer fixtures：dispatch scope、malformed input、renew false ownership loss、renew exception 后权威 fenced settlement、stale settlement、retry/finalize、reaper/maxTasks/timer cleanup，以及 fetch 子 cycle 安全日志。 |
| `apps/worker/src/modules/logging.ts` | 结构化日志：`emitStructuredLogStart` / `emitStructuredLogEnd`。 |
| `apps/worker/src/modules/health.ts` | Worker 健康检查：`runWorkerHealthCheck`。 |
| `apps/worker/src/modules/fetch-cycle.ts` | 显式 tenant scope fetch pipeline：`runFetchCycleForWorkspace()` 只执行传入 Organization/actor 的业务，不枚举 workspace、不创建 outer TaskRun，供 durable consumer 与多组织调度复用。 |
| `apps/worker/src/modules/organization-cycle.ts` | 多组织 fetch orchestrator：兼容默认 workspace、枚举 eligible Organization、稳定 actor、动态公平预算、per-org outer TaskRun、错误隔离与安全摘要；standalone `runFetchCycle()` 在此初始化独立总预算。 |
| `apps/worker/src/modules/{organization-cycle,main-cycle}.fixtures.ts` | 无 DB 行为 fixtures：覆盖公平预算、耗尽 skip、A 失败/B 继续、settlement 异常、单一 main deadline、drain→fetch 顺序及安全输出。 |
| `apps/worker/src/modules/organization-cycle-pg.fixtures.ts` | fail-closed disposable PostgreSQL integration：验证 ACTIVE actor、真实 tenant repository/destructive fencing、production workspace pipeline、TaskRun/UsageEvent/DeliveryLog 隔离。 |
| `apps/worker/src/modules/telegram-delivery.ts` | Telegram 投递 cycle：`runTelegramDeliveryCycle`、消息格式化、错误码提取。 |
| `apps/worker/src/modules/report.ts` | 报告生成：`runReportGeneration`、`runReportGenerationCycle`、规则/AI 生成、关键词提取。 |
| `apps/worker/src/index.fixtures.ts` | Worker runtime fixture：验证 rule/LLM noiseReason 的优先级与持久化前选择逻辑。 |
| `apps/worker/src/telegram.ts` | Telegram 共享 adapter：安全转义、长度限制、timeout/429/5xx 分类和 retry_after。 |
| `apps/worker/src/lib/safe-log.ts` | 安全错误日志：`formatSafeError` 仅提取 `name/message/code`，剥离敏感信息；`sanitizeErrorMessage` 替换 URL 凭证和绝对路径。 |
...
| `runMainWorkerCycle()` | 默认入口：在单一 deadline 下先 drain durable queue，再把 `getCycleRemainingMs()` 交给多组织 fetch；耗尽时只枚举 `SKIPPED_BUDGET`，不创建 TaskRun。 |
| `runTaskRunConsumerCycle()` | `--task-runs` 入口；recover expired leases 后 bounded claim SOURCE_FETCH/SOURCE_DISCOVERY/CONTENT_FETCH，heartbeat 并 fenced settle，返回 queue metrics。 |
| `runQueueWorker()` | `--queue-worker` 常驻入口；持续调用 bounded consumer，队列有任务时立即继续、空闲时按间隔轮询，并输出 `queue-drain` / `queue-worker-heartbeat` 安全结构化日志。 |
| `runEventSummaryRegeneration()` | 处理一个绑定 event/item/topic 的摘要任务：按需采集原文、执行一次 extraction、写 `READY` 或结构化失败状态；重复点击由 active idempotency 复用任务。 |
| `runFetchCycle()` / `runOrganizationFetchCycles()` / `runFetchCycleForWorkspace()` | standalone 入口初始化独立总预算；orchestrator 对全部 eligible Organization 串行公平调度并返回逐组织摘要；workspace 函数执行单租户完整 pipeline，不自行创建/结算 outer/durable TaskRun。 |
| `runSourceDiscoveryCycle()` / `runSourceDiscoveryForWorkspace()` | 前者保留 legacy standalone 审计 TaskRun；后者使用传入 workspace/taskRunId 执行业务与 usage，不嵌套创建或结算 TaskRun。 |
| `runAnalysisCycle()` | 分析：规则 relevance 可为采集失败条目创建空摘要占位事件；只有 `contentStatus=READY` 且 Markdown 非空时才写 `AI_EVENT_EXTRACTION` 并调用 LLM。AI/采集失败写结构化 summaryStatus，不回退 RSS/标题摘要；逻辑 AI 调用写 UsageEvent。 |
...
| `runInstantPushCycle()` | 遍历已启用组织，执行 effective-plan/凭证 gate、候选查询、原子 claim、Telegram 发送、有限重试和 TaskRun/UsageEvent 审计。 |
...
packages/sources fetchRssFeed()
  ↓
packages/db upsertFetchedItems() (content:encoded → safe Markdown / READY)
  ↓
apps/worker runArticleFetchCycle()
  ↓ packages/sources fetchArticleMarkdown() (Readability article.content + safe Markdown)
  ↓ packages/db updateItemContentCapture()
  ↓
CONTENT_FETCH TaskRun / Item(rawContent + contentStatus/source/fetchedAt/errorCode)
  ↓
Postgres
```
...
packages/core evaluateRelevance() → 占位事件可保留
  ↓ contentStatus=READY + non-empty Markdown only
packages/ai extractEvent() → quality validation
  ↓ packages/core calculateGravityScore()
packages/db upsertIntelligenceEventFromItem()
  ↓
IntelligenceEvent(status='UNREAD', summaryStatus=READY|failure status) + Item(status='ANALYZED')

Non-READY summary events
  ↓ dashboard/detail 可见并保留原文链接
  ✕ briefing / instant push / report evidence / semantic dedup
...
```text
FeedbackEvent(kind=READ|SAVE|DISMISS|EXPORT|CATEGORY_UP|CATEGORY_DOWN|MORE_LIKE_THIS|LESS_LIKE_THIS|SOURCE_QUALITY_UP|SOURCE_QUALITY_DOWN|SCORE_UP|SCORE_DOWN)
  ↓ apps/web action 或 apps/worker runPreferenceLearningCycle()
packages/db listRecentFeedbackSignals() 返回 FeedbackSignalRecord（携带 feedbackEventId/eventId/createdAt/topicId/source/category/value）
  ↓ packages/core generatePreferenceDeltas()（按 feedbackEventId 主键幂等 + topicId + preference key 隔离；30 天半衰期 applyTimeDecay）
...
### Instant Push（高分情报即时推送链路）

```text
Railway instant-push cron (*/15)
  ↓ runInstantPushCycle()
  ↓ Subscription effective plan + OrganizationCredential(TELEGRAM).instantPushEnabled + encrypted Telegram credential
  ↓ listInstantPushCandidates(ACTIVE source, score threshold, enabledAt)
  ↓ atomic claim InstantPushLog(SENDING)
  ↓ shared Telegram adapter
  ↓ SENT / FAILED(nextAttemptAt) / SKIPPED
```
