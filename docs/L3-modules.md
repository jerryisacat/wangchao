...
| `packages/db/prisma/migrations/0016_better_auth_schema_alignment/migration.sql` | Better Auth 1.6.23 core schema 对齐 + Issue #153 Lane 1 user lifecycle 数据层。新增 `Verification` 模型、`UserAccountStatus` 枚举、`User.emailVerified`/`image`/`name` SET NOT NULL（NULL 回填为 email）、`User.accountStatus`/`suspendedAt`/`suspendedReason`/`suspendEndsAt`/`deletionRequestedAt`/`deletedAt`/`lastLoginAt`/`lastActivityAt`、`Account.refreshTokenExpiresAt`/`idToken`/`scope`（`expiresAt` 保留并映射到 Better Auth `accessTokenExpiresAt`）、`User.accountStatus` 索引。 |
...
| `packages/db/src/repositories/user-lifecycle.ts` | Issue #153 Lane 1B-1 用户生命周期状态机。`getUserLifecycleStatus`/`suspendUser`/`reactivateUser`/`requestUserDeletion`/`markUserDeleted`/`recordUserLogin`/`recordUserActivity`。所有转换使用原子 `updateMany` + `where.accountStatus` 谓词；`count=0` 时再 `findUnique` 区分 `USER_NOT_FOUND` vs `INVALID_TRANSITION`；稳定错误码 `USER_NOT_FOUND`/`INVALID_TRANSITION`/`INVALID_REASON`，错误消息不泄漏 Prisma 内部信息。`lastLoginAt` 与 `lastActivityAt` 职责分离。`DELETED` 为终态。 |
...
| 文件 | 目的 |
|------|------|
| `apps/worker/src/index.ts` | Worker 入口。CLI 参数解析、cycle 调度、re-export。已拆分为 5 个子模块（见下）。 |
...
| `apps/worker/src/modules/fetch-cycle.ts` | Fetch cycle 编排器：`runFetchCycle`，调用所有子 cycle（analysis/dedup/preference/briefing/governance/telegram）。 |
