...
| `packages/db/src/crypto.ts` | AES-256-GCM 凭证加解密工具。`encryptCredential(plaintext, key)` / `decryptCredential(encrypted, key)` / `maskKeyHint(plaintext)` / `fingerprintKey(key)` / `cryptoSmokeTest()`。per-credential 随机 salt + scrypt KDF；密文格式 `salt:iv:ciphertext:tag`（base64，旧格式 `iv:ciphertext:tag` 保持向后兼容）。 |
