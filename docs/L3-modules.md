...
| `listBriefingsPage()` / `getBriefingMarkdownForDownload()` | organization-scoped 完整简报历史分页与下载读取。 |
...
- Preference learning 使用可解释规则：`SAVE/EXPORT` 提升 category/source 权重，`READ` 轻微提升，`DISMISS` 降低，`CATEGORY_UP/DOWN` 只改变 category 权重；归纳 Map 使用 `topicId + key` 复合键避免跨主题污染。后续可替换为 LLM 归纳，但必须保留 `PreferenceMemory.explanation` 可解释性。
...
| 文件 | 目的 |
|------|------|
| `packages/sources/src/index.ts` | 包公共出口：RSS source adapter、search provider、feed validation/probe、外链提取、`fetchArticleContent()`（基于 `@mozilla/readability` + `linkedom` 提取网页正文）。 |
...
│   ├── topics/[topicId]/page.tsx    # 主题详情页（状态、统计、管理操作）
│   ├── topics/[topicId]/edit/page.tsx # 主题编辑页（name/description）
...
| `apps/web/src/app/briefings/page.tsx` | organization-scoped 完整简报历史分页：总数、周期、UTC 日期窗口、更新时间、上一页/下一页和 Markdown 导出。 |
...
| `apps/web/src/app/preferences/page.tsx` | 偏好记忆页：权重、置信度、解释；置信度条提供 `progressbar` 语义。 |
| `apps/web/src/app/admin/settings/page.tsx` | Admin API Key 配置页（`/admin/settings`）：Tabs 布局（AI 凭证 / 搜索凭证）；展示 AI/search 凭证状态（脱敏 hint + key-value 布局 + 更新时间）；AI tab 提供新增/覆盖 AI Key（`aiApiKey`/`aiBaseUrl`/`aiProvider`/`aiModel`）、测试连接、清除凭证操作；Search tab 提供新增/覆盖搜索 Key（`searchApiKey`/`searchProvider`）、测试连接、清除凭证操作。OWNER/ADMIN 可访问，不展示完整 Key。 |
...
| `apps/web/src/lib/topic-source-data.ts` | 读取工作台、完整收藏/简报分页、单条情报详情和 dedicated workspace audit；成员/用量不再随每次主工作台请求白查。`DATABASE_URL` 未配置时抛出错误，不再静默降级为预览模式。 |
...
| `runAnalysisCycle()` | 分析：每 Item 写 `AI_RELEVANCE` TaskRun；有 AI runtime 时另写 `AI_EVENT_EXTRACTION`，失败落库后回退规则；最终 filtered/event-upserted 结果写回外层 TaskRun。 |
...
- Worker 声明的六类 TaskRun 必须有真实写入链路：source fetch/discovery、item relevance/LLM extraction、topic briefing、Markdown export；失败必须落 `errorMessage`，不能只写 stderr。
...
apps/web /admin/settings → /admin/usage
  ↓ getWorkspaceAudit()
ensureDefaultWorkspace() + assertMembershipRole(OWNER|ADMIN)
