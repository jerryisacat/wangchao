...
| 文件 | 目的 |
|------|------|
| `apps/worker/src/index.ts` | Worker 入口。默认主 cycle 先 drain durable TaskRun，再保留既有 fetch cycle；`--task-runs` 仅执行 queue consumer，其他 CLI cycle 不变。 |
...
| `apps/worker/src/modules/fetch-cycle.ts` | Fetch cycle 编排器：legacy `runFetchCycle()` 解析默认 workspace；`runFetchCycleForWorkspace()` 执行显式 tenant scope pipeline，供 durable consumer 与后续多组织调度复用。 |
...
| `runMainWorkerCycle()` | 默认入口：先运行 `runTaskRunConsumerCycle()` drain durable queue，再运行既有 default fetch cycle。 |
...
| `runFetchCycle()` / `runFetchCycleForWorkspace()` | 前者保留默认 workspace standalone 行为；后者对传入 workspace 执行完整抓取 pipeline，不自行创建/结算 durable TaskRun。 |
