...
| `packages/db/src/index.ts` | 包公共出口。 |
| `packages/db/src/repositories.ts` | Topic/Source/Worker/Dashboard/Preference/Briefing/Governance repository。提供 tenant/topic scoped 查询 helper，后续新增查询应优先放在这里或同包内的清晰模块中。 |
...
| `renderEventMarkdown(input)` | 渲染单条情报 Markdown 导出。 |
