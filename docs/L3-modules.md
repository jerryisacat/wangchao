...
| `enqueueTaskRun()` / `claimNextTaskRun()` / `renewTaskRunLease()` / `completeClaimedTaskRun()` / `failClaimedTaskRun()` / `yieldClaimedTaskRun()` / `recoverExpiredTaskRuns()` | Durable queue public API。Web enqueue PENDING；Worker 原子 claim + heartbeat；所有 settlement 受 owner/token/expiry fencing。 |
...
| `apps/worker/src/modules/task-run-consumer.ts` | Exact-type SOURCE_FETCH/SOURCE_DISCOVERY consumer：reaper、bounded claim、严格 input parser、workspace dispatch、lease heartbeat、指数退避和 fenced settlement。 |
...
| `runTaskRunConsumerCycle()` | `--task-runs` 入口；recover expired leases 后 bounded claim exact supported tasks，heartbeat 并 fenced settle，返回 queue metrics。 |
