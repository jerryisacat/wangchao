...
| `packages/db/prisma/migrations/0013_credentials_split/migration.sql` | 拆分 Subscription 凭证到 OrganizationCredential；删除冗余字段（secondaryItemIds/idempotencyKey/dismissedAt）；PaymentInvoice.amount Float→Decimal；Session.expiresAt 索引；FeedbackEvent 复合索引；migration 0002 IF NOT EXISTS 修复。 |
...
| `packages/db/src/repositories/instant-push.ts` | InstantPushLog 状态机、设置、候选查询、原子 claim、标记成功/失败。 |
...
Railway instant-push cron (*/15)
  ↓ runInstantPushCycle()
  ↓ effective plan + instantPushEnabled + encrypted Telegram credential
