...
| Queue Worker | `deploy/railway/queue-worker.railway.json` | 常驻消费 durable TaskRun，使手动摘要等命令秒级开始执行 |
...
| Report Cron | `deploy/railway/report-cron.railway.json` | 每 10 分钟扫描并生成待处理专题报告 |

Railway service 需要分别指向对应 config file path；全部应用服务连接同一个 Railway Postgres。
...
| Queue Worker | `apps/worker --queue-worker` | 常驻 durable TaskRun consumer，负责即时处理手动命令 | `pnpm railway:queue-worker:start` |
...
| Report Cron | `apps/worker --report-generation` | 待处理专题报告生成（每 10 分钟） | `pnpm worker:report-generation` |
...
| `WANGCHAO_DEPLOYMENT_MODE` | web | Yes | `self-hosted` or `commercial`; customer-facing Railway uses `commercial`. |
| `BETTER_AUTH_SECRET` | web | Commercial | ≥32-char strong random secret; never commit or log it. |
| `BETTER_AUTH_URL` | web | Commercial | Public HTTPS origin of the Web service. |
| `WANGCHAO_DEFAULT_*` | web, worker, seed | Self-hosted only | Default identity/workspace; not used to provision commercial customers. |
...
4. Create a Queue Worker service from the GitHub repo and set its config file path to `deploy/railway/queue-worker.railway.json`.
5. Create the Worker Cron, Source Discovery Cron, Instant Push Cron and Report Cron services with their matching config files.
6. Attach the Postgres database variable to all application services via Railway service reference (`${{Postgres.DATABASE_URL}}`), not hardcoded values.
7. Choose one Web deployment mode. For a customer-facing Railway service, set these three variables in one change so the fail-closed deploy does not see a partial configuration:

```text
WANGCHAO_DEPLOYMENT_MODE=commercial
BETTER_AUTH_SECRET=<openssl rand -hex 32>
BETTER_AUTH_URL=https://<public-web-domain>
```

For a no-login self-hosted deployment, set the default workspace variables instead:
...
8. For personal self-hosted use, set seed source variables before first Web deploy:
...
9. Deploy Web first. Its Railway pre-deploy validates the deployment mode, waits for Postgres and runs migrations; only self-hosted mode runs seed:

```bash
pnpm railway:web:predeploy
```

10. Deploy Queue Worker and cron services after Web succeeds.
11. Open the Web service health endpoint and confirm database and authentication status are `ok` in commercial mode:

```bash
curl -fsS https://<railway-web-domain>/api/health
```

12. Confirm an anonymous `/` request redirects to `/login`, register a fresh account, verify it receives an independent OWNER workspace, then confirm Queue Worker stays online and all Cron schedules are active.

### Railway Service Commands

六个 service-level config 使用各自的专用脚本（推荐路径）：

| Service | Build | Predeploy | Start | Cron |
|---|---|---|---|---|
| Web | `pnpm railway:build` | `pnpm railway:web:predeploy` | `pnpm railway:web:start` | — |
| Queue Worker | `pnpm railway:build` | `pnpm railway:worker:predeploy` | `pnpm railway:queue-worker:start` | —（常驻） |
...
| Report | `pnpm railway:build` | `pnpm railway:worker:predeploy` | `pnpm worker:report-generation` | `*/10 * * * *` UTC |
...
Role:       WANGCHAO_RAILWAY_ROLE=web|worker|queue-worker
```

Queue Worker 是常驻进程；其他 Worker Cron 进程设计为运行一轮后退出，由 Railway 按 schedule 重新启动。
...
The endpoint returns JSON with `service`, `status`, `generatedAt`, and dependency `checks`. Commercial auth configuration or DB failure returns `503`; self-hosted auth may be `skipped`.
...
- Worker cycle 输出 `cycle-start` / `cycle-end`；Queue Worker 额外输出低频 `queue-worker-heartbeat` 和有活动时的 `queue-drain`。详细字段见 `docs/railway-runbook.md` §2.3。
