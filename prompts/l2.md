...
# Task
Process the provided "New Items to Process" list into a highly curated, deduplicated, and technically accurate JSON feed. You will also receive a "Current Top News (For Deduplication)" list representing previously processed top stories. Your job is to score and summarize the new items, filtering out any that are duplicates or extremely similar to the existing top news.
...
- Compare the "New Items" against each other and against the "Current Top News".
- Group multiple articles about the same event.
- If a new item is covering the exact same event/announcement as an item in the "Current Top News", **DISCARD IT** (do not include it in the output json) unless the new item provides a significant technical update not present before.
...
# Output Format
Return a strictly valid JSON object. **ONLY include processed results for the "New Items to Process". DO NOT output the existing top news items.**
