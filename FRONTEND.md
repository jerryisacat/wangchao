...
| 层级 | 字体 | 规则 |
|---|---|---|
| 默认 UI | Geist Sans（本地 variable font，100-900） | 500 为标题默认；构建不依赖外部字体网络。 |
...
`/` 是无需登录的公开品牌入口，第一受众是研究者、投资人、创业者和行业从业者。页面使用独立 Marketing Shell 与 `max-width: 1200px` 编辑式画布，不沿用产品工作台的 920px 单列壳。

- Hero 使用上下堆叠构图：品牌主张、简短解释、单一主 CTA 与纯自动演示；不在首屏要求访客输入，也不调用匿名 AI/API。
- 自动演示采用“定义主题 → 观察信源 → 合并报道 → 提炼变化”四阶段轨道，以静态 fixture 展示 12 条示例报道归并为 3 个关键变化；概念流程最后过渡为真实情报卡。进入视口后只自动播放一次，页面隐藏时暂停，桌面时间轴约 11.5 秒、移动端约 9 秒，结束后停在结果并提供“重新播放”。
- `prefers-reduced-motion` 直接展示最终卡并隐藏无效重播动作；最终结果、价值文案与来源必须保留在服务端 HTML/静态 DOM 中。Marketing Shell 还必须在 `/` 的初始响应中提供 `<noscript>` 核心 fallback，不能让 RSC 挂载、脚本或动画成为理解产品的唯一方式。
- 等高线潮汐纹只用于演示画布和最终 CTA 的低透明度背景；不用粒子、霓虹、无限 marquee 或装饰性 Canvas/video。
- 移动端保持四阶段但纵向压缩；最终卡只保留“收藏 / 查看原文”两个动作。320/375/414/768/1024/1440px 均须验证无横向溢出、裁切或按钮挤压。
- `/pricing` 同属 Marketing Shell。匿名请求只能读取公共方案注册表，不得创建默认/用户工作区、查询订阅或读取 BYOK；登录用户的当前方案覆盖层失败时，公开定价主体仍应可用。

### 5.2 工作台首页：未读情报流

`/app` 的唯一核心任务是让用户处理未读情报。
...
工作台首页保留：未读情报流、主题筛选（全部 / 单主题，pill toggle）、情报卡片、快速动作（已读 / 收藏 / 减少这类 / 原文）、`新增主题` 入口。

工作台首页移出：组织与权限、用量审计、处理管线、信源治理详情、偏好记忆详情、新建主题大表单、大面积 KPI 指标卡。

### 5.3 App Shell 与导航

`AppShell` 根据 proxy 覆盖注入的受控 pathname 在三种壳之间切换：Marketing（`/`、`/pricing`）、Auth（`/login`、`/register`）和 Product（其余产品页面）。公开 Marketing/Auth 分支不得提前读取工作区；Product Shell 使用顶部导航（`components/layout/app-shell.tsx` + `top-nav.tsx`），亮色 sticky + `backdrop-blur` + 底边。

- 主导航（日常阅读 4 项）：未读情报 `/app`、简报 `/briefings`、报告 `/reports`、已保存 `/saved`。激活态 `bg-secondary text-secondary-foreground`。
- 右侧：新增主题 `/topics/new`（仅 `OWNER | ADMIN` 的 primary 药丸 CTA）+ 「更多」`DropdownMenu`；`MEMBER` 不渲染其无权执行的创建入口。
...
- 「更多」分组并按角色 / 登录态条件渲染：
  ...
  - 账户：账户与访问 `/account`、偏好记忆 `/preferences`、我的用量 `/usage`、方案与定价 `/pricing`。
  - 工作区（仅 `OWNER | ADMIN`）：工作区设置 `/admin/settings`、用量审计 `/admin/usage`。
  - 登出（仅 `authEnabled`）。
- 登录前路由（`/login`、`/register`）渲染极简品牌头，不显示应用导航；登录/注册缺省成功回跳为 `/app`，显式安全 `next` 继续保留。
...
草案确认页已经进入核对与编辑任务，不继续使用 `kinetic` Card 或水印。生成方式通过 `StatusBanner` 明示，表单按“名称与目标 / 主题画像 / 语言与简报偏好”拆成稳定的 `work` Card；重新生成与确认创建在移动端均占满可用宽度。草案过期、损坏和缺失状态使用统一 `EmptyState`，不保留内联样式或 bespoke class。
...
登录与注册页面使用复用的单一紧凑 `Card`，页面名称必须是语义 `h1`；字段沿用 MD3 filled field，密码输入提供 ≥44px 的显隐控制，主提交按钮占满宽度。登录 / 注册互跳是完整的 44px 次级按钮，不使用只有文字本身可点击的行内小链接。错误信息使用 `aria-live` 并与字段通过 `aria-describedby` 关联，加载状态必须有文字且禁用正在提交的字段。

- 认证开启时，卡片明确说明数据库 Session 边界；登录页把 `ACCOUNT_SUSPENDED` / `ACCOUNT_DELETED` 转换为稳定中文状态，不显示内部错误或原始 provider 文案。
- 认证关闭时，登录 / 注册页不得展示注定失败的凭证表单；使用 warning `StatusBanner` 明示“免登录自托管模式”，并提供整行“进入工作台”行动。
- `/account` 使用 `PageHeader` + `Card variant=work` 展示当前身份、账户状态、认证模式、工作区和 Membership role；正式认证模式提供明确登出动作，兼容模式展示认证关闭说明。邮箱、slug 等长值必须可换行，不得撑宽卡片。
...
2. **字体**：`layout.tsx` 使用仓库内 `geist` 包的 Geist Sans / Geist Mono variable font，不在构建期请求外部字体。
