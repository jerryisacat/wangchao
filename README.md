...
当前版本是**个人 / 单用户**形态，部署主路径是 **GitHub 自动同步到 Railway**：Railway 承载 Web、常驻 Queue Worker、各类 Cron 和 managed Postgres。多租户、团队权限、付费系统是后续阶段，不阻塞当前体验。
...
- **Telegram 投递**：管理员可配置简报投递；Plus/Pro 与自用模式还可开启高分情报即时推送，投递均有持久化审计。
- **手动重新生成摘要**：在情报详情页点击“重新采集”后创建单事件持久任务；Worker 下一轮优先消费该任务，重新采集原文并生成摘要，运行中的重复请求会复用同一任务。
...
当前个人版采用双 Worker 拓扑：Queue Worker 常驻消费手动 durable task；Worker/Discovery/Report/Instant Push 继续按“一轮任务后退出”的方式由 Railway Cron 启动。所有 application service 通过 GitHub `master` 自动部署并共享 Postgres。
...
| Queue Worker | `deploy/railway/queue-worker.railway.json` | 常驻消费手动抓取、发现和摘要重生成 durable task |
...
- 已完成 Railway Web + Postgres 生产 smoke test；GitHub integration 已连接，部署目标是 GitHub 自动同步到 Railway（Web + Queue Worker + 各类 Cron）。
