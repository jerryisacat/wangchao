...
当前 TypeScript 主路径的 V1 落地方式是：新建主题页只要求用户填写主题名称和描述；后端先生成包含 `keywords`、`entities`、`includeScope`、`excludeScope`、`importanceRules`、`languagePreferences`（摘要语言占位 + 术语规则）和 `digestStyle`（简报结构 + 详细程度 + 最大事件数）的初始 topic profile，再用内置信源包 `packages/db/seed-sources.json` 匹配候选 RSS/Atom。主题编辑页可读取并修改主题画像；保存使用 tenant-scoped update。在完成 i18n 适配前，AI extraction 的摘要及其他用户可见字段固定使用简体中文，不跟随原文语言，也不允许 topic profile 覆盖；`languagePreferences.outputLanguage` 暂固定保存为 `zh-CN`，待 i18n 后改为跟随界面语言。`languagePreferences.terminologyRules` 继续注入 system prompt；`digestStyle` 控制 daily briefing renderer 的结构、事件数上限和详细程度。规则 relevance 对 title/summary 做大小写不敏感的短语匹配：excludeScope 优先否决，keywords/entities/includeScope 作为可解释正信号；importanceRules 只进入 AI 评分，不伪装成 deterministic 规则。关键词另用于信源发现，完整画像进入 AI event extraction；AI 输入中的 topic name/description 始终来自 Topic 当前字段而不是 profile 内的重复快照。Worker 先把 RSS `content:encoded` 或 Readability `article.content` 转换为清洗后的 Markdown 快照，沿用 `Item.rawContent` 存储，并以 `contentStatus/contentSource/contentFetchedAt/contentErrorCode` 记录采集结果；只有 `contentStatus='READY'` 且 Markdown 非空时才允许 AI event extraction，正文截断到 8000 字符作为安全边界，RSS summary 不再作为事实正文 fallback。采集失败、内容不足、平台暂不支持或 AI 失败时仍可按可解释规则保留占位情报和原文链接，但 `summary` 保持空字符串，由 `summaryStatus` 驱动 UI 提示，且不得进入简报、即时推送、专题报告证据或语义去重。详情页“重新采集”只重置 durable 状态，由下一轮 Worker 处理，每事件每分钟最多请求 1 次。候选源必须经过真实 HTTP/HTTPS feed 验证并读取 feed title，验证通过后才写入 `Source.status='CANDIDATE'` 和 `SourceObservation.evidence`；候选源条目不进入正式分析；没有匹配或验证失败时仍创建主题，并提示用户稍后在信源管理页继续发现。
...
当前 TypeScript 主路径以 UTC 自然日作为 daily briefing 的稳定时间窗口：只聚合该窗口内新建、来自 `ACTIVE` source 且未被忽略/归档的正式情报；已读不会让同日重要事件从简报消失。`Briefing` 使用 `topicId + period + rangeStart` 唯一键 upsert，同一主题同一天重复运行 Worker 只刷新同一份简报及其事件集合，不新增重复记录；Web 简报中心分页提供完整历史和 Markdown 下载，支持按 DAILY/WEEKLY/MONTHLY 周期筛选。周报（UTC 自然周，周一开始）和月报（UTC 自然月）在每次 Worker fetch cycle 中同样幂等生成，使用各自 `period + rangeStart` 窗口。主题时间线页面按 `occurredAt` 倒序展示主题全部正式事件（含 merged sources）。可配置业务时区仍由后续能力扩展。
...
| `language_preferences` | 跟随界面语言的摘要语言占位与术语规则；i18n 前固定 `zh-CN` |
...
>
> 广告策略（Free 计划展示广告、自用模式可关闭）的完整定义见 `docs/business-model.md` §14，其数据基础为 `Subscription` 表的 `showAdsInSelfHosted` 字段。
